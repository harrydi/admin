...
class Smartaddons_Megamenu_Block_List extends Mage_Core_Block_Template
{
	...
	public function __construct($attributes = array()){
		...
		if($this->filterRouter()){
			if($this->_typeCurrentUrl == Smartaddons_Megamenu_Model_System_Config_Source_Type::CMSPAGE ){
				$item_id = $this->_itemCurrentUrl;
			}
			if($this->_typeCurrentUrl == Smartaddons_Megamenu_Model_System_Config_Source_Type::PRODUCT ){
				$item_id = 'product/'.$this->_itemCurrentUrl->getId();
			}
			if($this->_typeCurrentUrl == Smartaddons_Megamenu_Model_System_Config_Source_Type::CATEGORY ){
...
// 			Zend_Debug::dump($this->_allActivedId);die;
	...
	protected function _toHtml(){
		...
		//$template_file = "smartaddons/megamenu/block_template_default.phtml";
		// $template_file = 'smartaddons/megamenu/'. $this->_config["theme"].'.phtml';		
		$template_file = 'smartaddons/megamenu/megamenu.phtml';		
	...
	public function getItems(){
		...
		if($group_item->getStatus() == Smartaddons_Megamenu_Model_System_Config_Source_Status::STATUS_ENABLED){
	...
	public function getItemHtml2($item, $isFirstColumn =''){			// is testing
		...
		$this->setTemplate ('smartaddons/megamenu/item.phtml');
		return parent::_toHtml();		
	}
	public function getItemHtml($item, $isFirstColumn =''){	
		$prefix = Smartaddons_Megamenu_Model_System_Config_Source_Html::PREFIX;
...
// 		$activedClassName = ($this->isActived($item) AND $this->hasActivedType($item))?$prefix.'actived':'';
		...
		if($item->getAlign() == Smartaddons_Megamenu_Model_System_Config_Source_Align::RIGHT){
		...
		$title = ($item->getShowTitle()==Smartaddons_Megamenu_Model_System_Config_Source_Status::STATUS_ENABLED)?'<span class="'.$prefix.'title_lv'.$item->getDepth().'">'.$item->getTitle().'</span>':'&nbsp;';
		...
		if($item->getDepth() != $this->_config['start_level']){
			...
			if($item->getShowTitle()==Smartaddons_Megamenu_Model_System_Config_Source_Status::STATUS_ENABLED OR $this->hasIcon($item)){
	...
	public function isDrop($item){
		return ($item->getShowAsGroup()==Smartaddons_Megamenu_Model_System_Config_Source_Status::STATUS_DISABLED)?true:false;
	}
	public function isAlignRight($item){
		return ($item->getAlign()==Smartaddons_Megamenu_Model_System_Config_Source_Align::RIGHT)?true:false;
	}
	...
	public function hasActivedType($item){
		$activedType = array(
			Smartaddons_Megamenu_Model_System_Config_Source_Type::PRODUCT ,
			Smartaddons_Megamenu_Model_System_Config_Source_Type::CATEGORY ,
			Smartaddons_Megamenu_Model_System_Config_Source_Type::CMSPAGE ,
		);
		return (in_array($item->getType(),$activedType))?true:false;
	}
	public function hasLinkType($item){
		$linkType = array(
			Smartaddons_Megamenu_Model_System_Config_Source_Type::EXTERNALLINK ,
			Smartaddons_Megamenu_Model_System_Config_Source_Type::PRODUCT ,
			Smartaddons_Megamenu_Model_System_Config_Source_Type::CATEGORY ,
			Smartaddons_Megamenu_Model_System_Config_Source_Type::CMSPAGE ,
		);
	...
	public function hasConntentType($item){
		$contentType = array(
			Smartaddons_Megamenu_Model_System_Config_Source_Type::CMSBLOCK ,
			Smartaddons_Megamenu_Model_System_Config_Source_Type::CONTENT ,
		);
		return (in_array($item->getType(),$contentType))?true:false;
	}
	public function getLinkOfType($item){
		if($item->getType() == Smartaddons_Megamenu_Model_System_Config_Source_Type::EXTERNALLINK){
			return $this->filterUrl($item);
		}
		elseif($item->getType() == Smartaddons_Megamenu_Model_System_Config_Source_Type::PRODUCT){
			return $this->getProductLink($item);
		}
		elseif($item->getType() == Smartaddons_Megamenu_Model_System_Config_Source_Type::CATEGORY){
			return $this->getCategoryLink($item);
		}
		elseif($item->getType() == Smartaddons_Megamenu_Model_System_Config_Source_Type::CMSPAGE){
	...
	public function getContentType($item){
		if($item->getType() == Smartaddons_Megamenu_Model_System_Config_Source_Type::CMSBLOCK){
			return $this->getBlockPageHtml($item);
		}
		elseif($item->getType() == Smartaddons_Megamenu_Model_System_Config_Source_Type::CONTENT){
	...
	public function filterRouter(){
		...
		*/
		if(Mage::app()->getFrontController()->getRequest()->getRouteName() == 'cms'){
			$this->_typeCurrentUrl = Smartaddons_Megamenu_Model_System_Config_Source_Type::CMSPAGE ;
// 			$this->_itemCurrentUrl = Mage::getSingleton('cms/page')->getIdentifier() ;
		...
		*/
		if($current_page == 'catalog'){
			...
			if($this->getRequest()->getControllerName()=='product') {
				$this->_typeCurrentUrl = Smartaddons_Megamenu_Model_System_Config_Source_Type::PRODUCT ;
				$this->_itemCurrentUrl = Mage::registry('current_product') ;	
				return true;			
			}//do something
			if($this->getRequest()->getControllerName()=='category'){
				$this->_typeCurrentUrl = Smartaddons_Megamenu_Model_System_Config_Source_Type::CATEGORY ;
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('megamenu_cfg/advanced/include_jquery')){
				...
				$import_str .= $jsHelper->includeSkinScript('smartaddons/megamenu/js/jquery-1.5.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('smartaddons/megamenu/js/jsmart.noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		
		if (null == Mage::registry('jsmart.megamenu.js')){
			// add script for this module.
			// $import_str .= $jsHelper->includeSkinScript('smartaddons/megamenu/js/jquery.hoveraccordion.js');
			$import_str .= $jsHelper->includeSkinScript('smartaddons/megamenu/js/js.js');
	...
	private function _getResizedImage($Obj, $width, $height, $quality = 100) {
		...
		if(file_exists($imageResized)){
			...
			return $this->getSkinUrl('smartaddons/megamenu/images/no_image.gif');
		}
	}	
}
